    ...
    public class MagioObjectEditor : Editor
    {
                    ...
                    if (magioObj.enableMaterialAnimation && !enableMaterialAnimation.hasMultipleDifferentValues)
                    {
                        ...
                        foreach (Object magioO in targets)
                        {
                                ...
                                foreach (Renderer rend in rends)
                                {
                                    ...
                                    for (int i = 0; i < comps.Count; i++)
                                    {
                                        ...
                                        if (comp == null)
                                        {
                                            ...
                                            {
                                                if (eff.magioShaderEffectMode == MagioObjectEffect.MagioShaderEffectMode.Dissolve_ONLY_IN_PRO_VERSION)
                                                {
                            ...
                            else
                            {
                                ...
                                if (magioObj.magioShaderEffectMode == MagioObjectEffect.MagioShaderEffectMode.Dissolve_ONLY_IN_PRO_VERSION)
                                {
                                    magioShaderDissolveCount++;
                                }
                        ...
                        if (magioShaders > 0)
                        {
                            ...
                            {
                                EditorGUILayout.Space();
                                EditorGUILayout.LabelField("ONLY AVAILABLE IN PRO VERSION", EditorStyles.boldLabel);
                            }
                        }
        ...
        public void CheckForErrors()
        {
            ...
            foreach (Object magioO in targets)
            {
                ...
                if (eff.targetGameObject.GetComponentsInChildren<Collider>().Length == 0 && !eff.beginEffectOnStart)
                {
