...
namespace Magio
{
    ...
    public class RaycastNullify : MonoBehaviour
    {
        ...
        public void CastRayCastNullify()
        {
            ...
            RaycastHit[] hits = Physics.SphereCastAll(castPoint, raycastRadius, transform.InverseTransformPoint(direction), maxDist, mask);
            if (hits.Length > 0)
            {
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                Gizmos.DrawRay(transform.position, transform.InverseTransformPoint(direction));
            }
        }
    }
}
