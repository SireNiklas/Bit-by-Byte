...
namespace Magio
{
    ...
    public class MagioEngine : MonoBehaviour
    {
        ...
        [Tooltip("Where to parent the effects")]
        ...
        [Tooltip("---Remember to backup your terrain data before using! Magio modifies terrain data and data can be lost in case of blue screen!--- Is Magio compatible with Unity Terrain? If you do not want terrain trees to burn enabling this will affect negatively on performance")]
        public bool unityTerrainCompatible = false;

        [Tooltip("Is Magio VSPro Compatible? If you don't want to affect Vegetation Studio Vegetation leave this off, otherwise it will negatively impact your performance")]
        public bool VegetationStudioProCompatible = false;

        [Header("Performance")]

        [Range(0.5f, 60)]
        [Tooltip("How often do we check for collisions with effect in second? Smaller = more efficient, bigger = more realistic. Value 5 = 5 times per second.")]
        ...
        [Tooltip("Allows modifying constant effect attributes on runtime. This will impact performance. Disable if you do not need to scale the object/modify flames on runtime to gain small performance boost.")]
