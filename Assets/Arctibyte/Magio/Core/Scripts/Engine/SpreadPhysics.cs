    ...
    public class SpreadPhysics : MonoBehaviour
    {
        ...
        private void ProcessCollidedObject(Collider other, Vector3 dist, Vector3 center)
        {
            ...
            if (magioObjProps)
            {
                ...
                {
                    if (magioO != null && magioO != myMagioObj && magioO.targetGameObject != myMagioObj.targetGameObject && !magioObjsAndPositions.ContainsKey(magioO) && magioO.effectClass == myMagioObj.effectClass)
                    {
