...
public class RaycastTest : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit) && Input.GetKey(KeyCode.Mouse0) && Time.time >= timestamp)
        {
            Debug.Log("HIT!");
            Debug.DrawRay(ray.origin, ray.direction * hit.distance, Color.green);

            GameObject projectile = Instantiate(_projectilePrefab, Camera.main.transform.TransformPoint(Vector3.forward * 1), transform.rotation);
            Itemcount++;
